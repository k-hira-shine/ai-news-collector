...
【重要】すべての提案は「今日・昨日発表・発生した最新情報」を起点にすること。
- 以前からある技術・サービスを単に紹介するのではなく、「今日何が新しくなったか・何が起きたか」を必ずタイトルや文案に明示すること
- 例：「Claude Codeがある」ではなく「Claude Codeに今日『agent view』が追加された」
- 鮮度が命。「速報」「たった今」「〇月〇日発表」などの言葉を積極的に使うこと
...
1. youtube_ideas: YouTube動画企画案を3件
   - title: 「今日発表」「速報」「たった今」など鮮度を示す言葉＋具体的な新情報を含むタイトル（数字・固有名詞・新機能名を必ず入れる）
   - hook: 冒頭30秒で「なぜ今日この動画を見るべきか」を伝えるフック（いつ・何が・どう変わったかを明示）
   - reason: なぜ今この動画が伸びるか（何日前に発表・何件バズっているなど具体的な根拠）
   - angle: 他のAI系チャンネルとの差別化ポイント・独自アングル
   - urgency: "high"（今週中）/ "medium"（今月中）/ "low"

2. x_post_ideas: X投稿ネタを5件
   - theme: 投稿テーマ（必ず「今日発表/今日判明」した情報を起点にすること）
   - draft: 実際に投稿できる文案（日本語280文字以内。「速報」「たった今」「〇月〇日発表」など鮮度を示す表現を入れる。改行・絵文字も含めてよい）
   - format: "スレッド" / "単発" / "画像付き"
   - expected_reach: "high" / "medium" / "low"

3. business_insights: ビジネス活用・マネタイズの示唆を3件
   - insight: 今日の新発表・新情報から読み取れるビジネスチャンスや市場の変化（何が新しく起きたかを明示）
