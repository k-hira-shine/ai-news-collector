...
<section class="overview" id="overview">
  ...
  <div class="ov-hero">
    <p class="ov-tagline">「<strong>なんでも入力して、なんでも作れる</strong>」モデル — まずは動画から</p>
  ...
  <div class="overview-grid">
    ...
    <article class="ov-card">
      <h3>🤔 そもそも何が新しいの？</h3>
      <p class="ov-card-desc">去年の <strong>Nano Banana</strong>（画像生成）の動画版です。テキスト・画像・音声・動画を好きに組み合わせて入力し、そこから高品質な動画を生成・編集できます。既存の動画生成 AI（Veo など）はテキストや画像 → 動画だけでしたが、Omni は<strong>動画を入力として動画を編集・リミックス</strong>できる点が最大の違いです。</p>
    </article>

    <article class="ov-card">
      <h3>📦 モデルの種類</h3>
      <ul>
        <li><strong>Gemini Omni Flash</strong>（今すぐ使える）<br>— 最初にリリースされた実用モデル。動画生成・編集に対応。</li>
        <li><strong>Gemini Omni Pro</strong>（近日公開予定）<br>— キーノートで言及のみ。詳細は未発表。</li>
        <li>将来的には動画以外（画像・音声）の出力にも対応予定。</li>
      </ul>
    </article>
    ...
    <article class="ov-card">
      <h3>🎯 できること（公式より）</h3>
      <ul>
        <li>テキスト・写真・音声・動画を混ぜて動画を生成</li>
        <li>「背景を変えて」「この人物を追加して」など<strong>会話で動画を編集</strong></li>
        <li>人物・キャラクターの顔や声を複数シーンで統一（<strong>キャラクター一貫性</strong>）</li>
        <li>自分の顔・声を使った AI アバター動画の作成</li>
        <li>生成動画には自動的に <strong>SynthID 透かし</strong>が入り、AI生成と検証可能</li>
      </ul>
    </article>
    ...
    <article class="ov-card">
      <h3>📱 どこで使えるか</h3>
      <ul>
        <li><strong>Gemini アプリ</strong> — Google AI Plus / Pro / Ultra プラン（有料）</li>
        <li><strong>Google Flow</strong> — 映像クリエイター向けの AI 制作スタジオ。Google AI サブスクで利用可。</li>
        <li><strong>YouTube Shorts / YouTube Create</strong> — 18歳以上なら<strong>無料</strong>で利用可。</li>
        <li><strong>API（開発者向け）</strong> — 「数週間以内」に公開予定と発表済み。</li>
      </ul>
    </article>

    <article class="ov-card">
      <h3>⚠️ 現時点の制限（公式・ユーザー報告）</h3>
      <ul>
        <li>生成できる動画は <strong>最大10秒</strong>（公式は「モデルの限界ではなく設計上の判断」と説明。延長は近日予定）</li>
        <li>レート制限が厳しめ（Pro プランでも数プロンプトで消費量が多いという報告あり）</li>
        <li>スタイル転送・物理表現（ジャンプ・回転など）は苦手な場合あり</li>
        <li>Seedance 2.0 との比較では「見た目は良い。ただ演出・カメラワークは Seedance の方が上」という実使用レビューも</li>
      </ul>
    </article>

    <article class="ov-card">
      <h3>🔥 実際に使われている場面（ユーザー報告）</h3>
      <ul>
        <li><strong>動画編集・リミックス</strong>（52件）— 背景変更・スタイル変換・別アングル生成など</li>
        <li><strong>Google Flow × Agent モード</strong>（46件）— 1プロンプトで複数ショットを自動生成</li>
        <li><strong>UGC 広告・商品動画</strong>（30件）— 1本の素材から複数バリエーションを量産</li>
        <li><strong>他モデルとの比較検証</strong>（20件）— Seedance 2.0 と同条件で比較するレビューが多数</li>
      </ul>
    </article>
...
.ov-hero {{ background: linear-gradient(135deg, rgba(66,133,244,0.15), rgba(167,139,250,0.12)); border: 1px solid rgba(167,139,250,0.35); border-radius: 12px; padding: 18px 22px; margin-bottom: 16px; }}
.ov-tagline {{ font-size: 1.05rem; color: var(--text); line-height: 1.55; margin-bottom: 8px; }}
.ov-source {{ font-size: 0.8rem; color: var(--muted); }}
...
.ov-card-desc {{ font-size: 0.87rem; color: #cbd5e1; line-height: 1.65; }}
.ov-card ul {{ padding-left: 1.1rem; font-size: 0.86rem; color: #cbd5e1; }}
.ov-card li {{ margin: 6px 0; line-height: 1.55; }}
