...
- `official_count` の推移（月曜朝便で非ゼロに戻るか）。
- `daily_check.py` 1本での運用を継続。深掘りが要るときのみ個別ツール。

### F. このセッションのコミット
| # | commit | 内容 |
|---|--------|------|
| 1 | （本記録） | docs: 2026-06-21 日次チェック（全項目合格・「収集品質」実値切替＝変更1ロールアウト確認） |
