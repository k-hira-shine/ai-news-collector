...
> このセッションでやったこと:
> 1. 「今日のエラーチェック」として `daily_check.py` / pytest / 構文チェック / CSS漏れチェックを実施。
> 2. `AI News Collector` の最新定期runが `cancelled` で、2026-06-25朝便が未完了と判明。
> 3. 原因が収集/分析コードではなく、CIのフォントインストールであることをGitHub Actionsログから特定。
> 4. workflowを修正してpushし、手動 `workflow_dispatch mode=full` で今日分を補完。
> 5. 追加の安定化（OS固定、step timeout、daily_checkのActions表示改善）を実装・push。
> 6. ローカルの古い生成物差分を整理し、`main` を `origin/main` に追従。最終的に作業ツリーclean。

### A. 発覚時の状態

- 日次チェック初回結果: `GitHub Actions` のみ要確認、他は合格。
- 対象run: `AI News Collector` run `28117574739`
  - event: `schedule`
  - created: `2026-06-24T17:35:29Z`（JST 2026-06-25 02:35頃）
  - conclusion: `cancelled`
  - job timeout: 45分
- ローカル/remoteとも当初は今日分が未生成:
  - `data/daily/2026-06-25.jsonl` なし
  - `data/analysis/2026-06-25_morning.json` なし
  - 最新は `2026-06-24` まで

### B. 原因

`collect.yml` の `Install Japanese fonts (Noto CJK)` が以下を毎回入れていた。

```bash
sudo apt-get install -y --no-install-recommends fonts-noto-cjk fonts-noto-cjk-extra fonts-noto-color-emoji
```
...
Get:2 ... fonts-noto-cjk ... [61.2 MB]
Get:3 ... fonts-noto-cjk-extra ... [145 MB]
##[error]The operation was canceled.
```

`fonts-noto-cjk-extra` の145MB取得がGitHub runner側で極端に遅く、`Run collector` に入る前にjob全体の45分timeoutへ到達した。つまり、**X収集・Gemini分析・ページ生成のコード障害ではない**。

### C. 実施した修正

| commit | 内容 | 目的 |
|---|---|---|
| `c76ab02` | `fonts-noto-cjk-extra` を削除し、`fonts-noto-cjk` と `fonts-noto-color-emoji` だけにした | 日本語基本表示は残し、巨大追加フォント取得でのtimeoutを避ける |
| `ce11081` | `runs-on: ubuntu-24.04` に固定 | `ubuntu-latest` の将来変更による予期しないCI差分を避ける |
| `ce11081` | `Install Japanese fonts` に `timeout-minutes: 5` | aptが詰まってもjob全体45分を食い潰さず早期に原因を出す |
| `ce11081` | `Install Playwright Chromium` に `timeout-minutes: 10` | 同じく依存取得のハングを早期切り分け |
| `ce11081` | `daily_check.py` のActions警告を `failure` 固定から `conclusion → workflow MM-DD HH:MMJST` 表示へ変更 | `cancelled` と `failure` を朝の確認で誤読しない |
...
```bash
gh workflow run collect.yml --repo k-hira-shine/ai-news-collector --ref main -f mode=full
gh run watch 28124849016 --repo k-hira-shine/ai-news-collector --exit-status
```
...
- conclusion: `success`
- duration: `13m27s`
- headSha: `c76ab029279209ccb0a4bed09aea6c817bdf4232`
- 問題だった `Install Japanese fonts` は通過。
- `Run collector`、ページ生成、`Commit & push data`、Pages deploy まで全成功。

生成・pushされた主な今日分:

- `data/daily/2026-06-25.jsonl`
- `data/analysis/2026-06-25_morning.json`
- `data/hn/2026-06-25.jsonl`
- `data/logs/2026-06-25.jsonl`
- `docs/diagrams/2026-06-25-morning.html`
- `docs/diagrams/2026-06-25-morning.png`
- `docs/index.html`
- `docs/home.html`
- `docs/tools.html`
...
- `ea63a8f data: 2026-06-24_1953`

### E. 検証

ローカル/一時worktreeで確認済み:

```text
workflow YAML OK
tests/test_daily_check.py: 49 passed
pytest: 166 passed, 1 warning, 17 subtests passed
scripts/check_incident_css_leak.py: OK
scripts/check_buzz_health.py --staleness-only: OK
```

最新 `origin/main` / ローカル `main` での最終 `daily_check.py`:

```text
✅ GitHub Actions: 直近5種success・期待4種cadence内
✅ Apify: 月換算 $3.86（窓 2026-06-18〜2026-06-24・上限 $12・on_target）
✅ Gemini: 2026-06-25 $0.171/日・直近4日平均で月換算 $6.6
✅ Buzz: 最終 2026-06-24（27h前）・status=pass・overlap=100.0%
✅ 収集品質: legal_rss=11/96（11%）・feeds 8/8健全・must_follow=77・x_valid=177
✅ 分析構造: morning top=10/cat=5/action=5/fallback=0（item=96）
✅ 全項目合格
```

### F. ローカル作業ツリー整理

作業中にremoteの自動生成commitとローカル生成物変更が競合し、一時 `autostash` ができた。最終的に:

- 古い生成物差分は破棄。
- `git merge --ff-only origin/main` でローカル `main` を `ce11081` へ追従。
- 一時 `autostash` は削除。
- `git status --short` は空。
- 既存の古い別件stashは2件残置:
  - `stash@{0}: On main: pre-reset-stash`
  - `stash@{1}: WIP on main: 305c92a fix: batch Apify timeline calls and add cost monitoring`
...
```bash
gh run list --repo k-hira-shine/ai-news-collector --workflow collect.yml --limit 5 \
  --json databaseId,status,conclusion,createdAt,event,url
gh run view <RUN_ID> --repo k-hira-shine/ai-news-collector --json jobs,conclusion,status,createdAt,updatedAt,url
gh run view <RUN_ID> --repo k-hira-shine/ai-news-collector --log-failed
```

切り分け:

- `Install Japanese fonts` で失敗: apt/runner依存取得問題。今は5分で早期失敗する。必要ならフォントインストール自体の削減またはキャッシュ化を検討。
- `Install Playwright Chromium` で失敗: Playwright依存取得問題。今は10分で早期失敗する。必要ならブラウザキャッシュや依存分離を検討。
- `Run collector` で失敗: Apify/Gemini/収集ロジックを見る。
- `Commit & push data` で失敗: 他workflowとのpush競合、`.github/scripts/push_data.sh`、concurrencyを確認。
- `Deploy to GitHub Pages` で失敗: `PAGES_DEPLOY_KEY`、公開先repo `k-hira-shine/ai-news-dashboard`、SSH pushを確認。

### H. 明日以降の合格条件

- `daily_check.py` が全項目合格。
- GitHub Actions欄が `直近5種success・期待4種cadence内`。
- `data/daily/YYYY-MM-DD.jsonl` と `data/analysis/YYYY-MM-DD_morning.json` が作成済み。
- `collect.yml` の朝便が `Install Japanese fonts` を数分以内に通過し、全job成功。

## 次回アクション（日付つき・最新版）

0. ~~🔴 **最優先（2026-06-24 朝便後）＝must_follow per-account修正がスケジュール起動でも効くか確認**~~ ✅ **完了（2026-06-24）＝スケジュール朝便でも回復を確定**:
   6/24朝便（collect 02:46 JST・schedule）で **must_follow=58・items=83・x_valid=259**＝合格基準（mf 50〜90台・items 80前後）を満たし回復確定（修正前は mf=8・items=27）。
   手動dispatchだけでなく**自動便でも per-account run 修正が有効**と確定＝本件クローズ。daily_check全6項目合格(exit 0)。
   - ただし**新フィールド `must_follow_critical_zero` の初記録で critical 10中5が0件**＝`AnthropicAI / GoogleAI / AIatMeta / huggingface / DarioAmodei`。総量mf=58は健全だが**最重要群が初日から黙り**＝下#0bで注視。
...
0b. ⏳ **critical沈黙advisoryの有効化確認（6/26頃）＋初日5件0の追跡**: 追補4の個別沈黙検知は新フィールド
   ...
   6/23朝便（collect run 03:28 JST・schedule）で **x_valid=80**（崩落時39〜44）・**legal_rss=4**・**must_follow=8**・**official=4**＝
   飢餓していた日本語/法務クエリが復活。**クエリ毎独立run修正がスケジュール起動でも有効と確定**（手動本番テストの251件回復だけでなく自動便でも回復）。
   - 「収集量急減」advisoryは沈黙＝健全。※`items_collected=27` は最終dedup後の値で当日のニュース量依存。actor修正の効きは raw な `x_valid`（44→80）と per-query復活（legal/must_followが0でない）で判定＝回復。
   - **"法務一色化"の赤は解消**: `legal=4/27=15%`。6/22の54%は取り直しによる飢餓2日分バックログ放出の一過性と確定（`LEGAL_DOMINANCE_RATIO` 再考は不要）。
   - **JURIST修正が初反映**: `feeds 8/8健全`（6/22は7/8）→ JURIST TODOクローズ。IPWatchdogも failed=0（6/22・6/23連続）＝再発なしなら6/24クローズ。
...
## 2026-06-23 追補4：検知の残穴を閉塞（じわ減り＋単一criticalアカウント沈黙）＝検知4層化

> ユーザー指示「これでもう同じミスは起こらない？」→「穴を塞いで」。追補3で**単日の崩落（崖）**は
> 絶対フロアで塞いだが、**フロアの上で続く慢性劣化**と**合計では隠れる単一アカウントの黙り**が残っていた。
> この2つを閉じ、検知を4層構成にした。

### A. 残っていた2つの穴
1. **じわ減り（chronic erosion）**: フロア(例 must_follow≥25)の上で `82→40→35` と続く中規模劣化は崖判定では拾えない。
   本来拾うべき相対(直近中央値比)判定は、劣化が続くと**ベースラインごと汚染されて盲目**（追補3で実証済みの弱点）。
2. **単一criticalアカウント沈黙**: OpenAI等10個のcriticalのうち1つが黙っても、**合計must_followが健全なら隠れる**。

### B. 対処（commit `2cdc0b1`コード / `7251a49`doc）
| 穴 | 対策 | 重大度 | 実装 |
|---|---|---|---|
| じわ減り | **固定の健全リファレンス**比で痩せが2連続→要確認。汚染されない固定基準が肝。 | fail | `daily_check.py`: `HEALTHY_REF={x_valid:300,items:110,must_follow:80}`・`EROSION_FRACTION=0.5`・`EROSION_STREAK=2` |
| critical沈黙 | per-account件数を記録→3連続ゼロのcritical handleを⚠表示 | advisory（個人は数日沈黙し得る） | `collector.py`(per_account記録+`must_follow_critical_zero`)→`main.py`(永続化)→`daily_check.py`(`CRITICAL_DARK_STREAK=3`) |

### C. 検知の最終形（4層）
1. **絶対フロア**（追補3・full便/fail）＝単日の崩落（崖）。`x_valid<120 / items<40 / must_follow<25`。
2. **じわ減り**（追補4・full便/fail）＝固定リファレンス比50%が2連続（傾斜）。
3. **critical沈黙**（追補4・advisory）＝単一アカウントの個別断（局所）。
4. **横断不変テスト**（追補3・CI/fail）＝`collect_x_twitter` の全actor呼び出しが `searchTerms` 1語（コード取りこぼし防止）。
＋既存の相対急減/収集量advisory/法務一色化/feed健全性/分析構造も併存（多重防御）。

### D. テスト・検証
テスト+7（じわ減り3・critical沈黙3・collector per-account1）＝**全150件green**。実データ(91/82/211)は最新便が健全で
streakが切れるため**誤発火なし＝緑維持**を確認。

### E. 既知の限界（残リスク・ポストモーテム§5に明記）
- フロア/リファレンス値は現行のクエリ数・アカウント数・収集窓に依存＝変更時は再較正（定数は`daily_check.py`冒頭に集約・根拠コメントあり）。
- critical沈黙は履歴3件（6/25頃）から有効化（次回#0b）。
- **件数に現れない質的劣化**（取得の質低下・出力スキーマ変更等）は依然どの検知でも捕まらない＝既知の限界。

### F. 本セッション通しのコミット台帳（6/23・古い順）
| commit | 内容 |
|---|---|
| `986689c` | fix: must_follow をアカウント毎の独立runに（残バグ修正・追補2） |
| `a8dacf3` | fix(detect): 経路別の絶対フロア＋x_valid表示＋横断不変テスト（追補3） |
| `53b19ff` | docs: handoff 追補3（RCA） |
| `f7a7eb1` | docs: ポストモーテム新規作成 |
| `2cdc0b1` | feat(detect): じわ減り＋critical沈黙の検知追加（追補4） |
| `7251a49` | docs: ポストモーテムに慢性側閉塞を追記＋レビュー反映 |
（※朝の日次チェック分 `26d4491`/handoff更新等も同日。完全な要約は本ファイル冒頭の追補1〜4チェーン）

---

## 2026-06-23 追補3：RCA＝なぜ取りこぼし＆検知失敗が起きたか／恒久対策（経路別フロア）

> ユーザー指示「なぜこの事態になったのか、ミスを見抜けなかったのか、異常検知がザルだった原因を徹底的に潰せ」への回答。**3層の独立した失敗が重なった**。各層に恒久対策を入れた。

### A. 失敗の3層

**① コードの取りこぼし（incomplete fix）**
6/22の障害対応で `collect_x_twitter` を per-query 化したが、この関数には X検索が**2経路**ある:
- `search_queries`（検索語ループ）→ per-query 化した ✅
- `must_follow`（必須アカウント）→ `searchTerms=[全30アカウント]` の**バッチのまま放置** ❌

両者は**構造的に同一の呼び出し**（バッチ `searchTerms` + 共有 `maxItems`）なのに、片方しか直さなかった。原因＝修正が「検索ログに見えた症状」だけを対象にし、**全call siteを grep で洗わなかった**。must_follow の症状は `must_follow_count` が0でなく"薄い"(8件)だったため目に入りにくかった。

**② 異常検知がザルだった（baseline poisoning）**
`evaluate_collection_quality` の急減検知は全て**「直近full便の中央値」比の相対判定**だった。劣化が数日続くと**ベースライン自体が劣化値で埋まり、崩落値すら平常に見える**。実データで確認:
```
6/23朝便を最新とみなした時の must_follow急減判定:
  baseline = median([5, 6, 17, 8]) = 7   ← 全部が障害期間中の値（汚染）
  判定: cur=8 < 0.2*7=1.4 ? → False（発火せず）
収集量急減(advisory)も baseline=median([7,12,84,27]) で同様に盲目。
```
＝**相対のみの異常検知は、劣化が持続的／メトリクス履歴の開始直後に起きると構造的に検知できない**。今回 actor 障害が must_follow_count の永続化開始(6/20)とほぼ同時に劣化を始めたため、健全ベースラインが一度も貯まらなかった。加えて閾値0.2（5倍減でやっと）も緩すぎた。

**③ 私（担当）の判断ミス（aggregate trap）**
6/23朝の#0確認で `x_valid=80`・daily_check全緑だけを見て"回復"と早合点。**健全ベースライン(206〜528)と突き合わせず、経路別(検索/ must_follow)にも切り分けなかった**。緑だったのは②の盲点が原因で、私の誤断と検知のザルは**同じ根（劣化後ベースライン基準で総量を見た）**。

### B. 恒久対策（すべて実装・push済み `a8dacf3`）

| 層 | 対策 | 効果 |
|---|---|---|
| ② | **経路別の絶対フロア**を本判定に追加: `FULL_X_VALID_FLOOR=120`/`FULL_ITEMS_FLOOR=40`/`FULL_MUST_FOLLOW_FLOOR=25`（full便のみ・健全実測下限から設定） | 相対と違い**履歴汚染に強く劣化初日から検知**。must_follow単独の崩落も経路別フロアで捕捉 |
| ③ | 収集品質detailに **`x_valid` を表示** | 「総量は出てるが must_follow が痩せている」型を目視でも切り分け可能 |
| ① | **横断不変条件テスト**＝`collect_x_twitter` の全actor呼び出しが `searchTerms` 1語であることを固定 | 3本目の経路がバッチで足されても即CI落ち＝取りこぼし再発防止 |

**検証（実ログ再生）**: 過去3障害日を新検知に通すと全て初日から ❌ になり、修正後データは ✅:
```
6/21 崩落1日目  items7/mf5/x39   → ❌（旧は素通し）
6/22 崩落2日目  items12/mf6/x44  → ❌（旧は素通し）
6/23 must_follow残バグ items27/mf8/x80 → ❌（旧は素通し＝私が見逃した値）
6/23 修正後     items91/mf82/x211 → ✅合格
```
テスト全143件green（フロア7件＋横断不変1件を新規追加、相対前提だった既存テストは新ポリシーに整合）。

### C. 残る注意点（フロア値の運用）
フロアは「健全便の実測下限と障害値の間」に置いた固定アンカー。将来 X の平常収集量が構造的に変われば誤検知し得る＝その時は実測に合わせて再較正する（定数は `daily_check.py` 冒頭に集約・コメントで根拠明記）。相対の急減検知/advisory も残してあり、フロア上での大きな変動はそちらが拾う（二段構え）。
...
## 2026-06-22 日次チェック（全項目合格 / 改善Bロールアウト確認 / JURIST死URL修正 / GH_PAT根本対応を完全クローズ）

> このセッションでやったこと（実施 6/22 04:50〜・2部構成）:
> **第1部＝日次チェック**: ①`git pull --rebase`で6/22朝便分（analysis/daily/logs/buzz等）を取得 → ②`daily_check.py`で合否1画面確認（全✅・6項目＋TODOフッタ） → ③6/22(月)朝便で**改善B（法務RSSフィード健全性）が初ロールアウト**し収集品質項目に `feeds 7/8健全` が初表示されたことを確認（6/21実装=改善Bのロールアウト成功）。
> **第2部＝点検と修正**: ④「他に気になる点はないか」の点検で改善Bが出した**非健全1枠の正体を追跡** → ⑤**JURISTの設定URLが死URL（200だが空スタブ）**と特定し実フィードへ修正、IPWatchdog 504は自己回復を確認。**この修正は改善Bが無ければ永久に気づけなかった盲点＝改善Bの初成果。**
> **第3部＝GH_PAT根本対応の完遂**: ⑥バックログ唯一の本物だった「PAT公開埋め込み事故」を**サーバー側中継(Cloudflare Worker)化**で構造から解消。コード実装→デプロイ→secret格納→実起動テストまで同日で完走（ユーザー操作はブラウザのAuthorizeクリックとダッシュボードでのsecret貼付のみ＝ターミナル不要だった）。⑦ついでに法務Xクエリ追加を見送り削除＝バックログ4→2件。
>
> **TL;DR（次担当者が最初に押さえる4点）**: (1) 6/22は**全項目合格**。(2) **JURIST死URLを修正済み**＝6/23朝便で `feeds 8/8健全` を確認したらクローズ。(3) 改善A（must_follow急減）は**6/24(水)で初有効化**、それまで沈黙が正常。(4) **GH_PAT事故は根本対応で完全クローズ**＝PATはもう公開ページに無くCloudflareの金庫のみ・buzzボタンも復活（節F）。残バックログは2件（JURIST/IPWatchdog）で両方手当て済み＝待つだけ。

### A. daily_check.py の結果（合否1画面・全✅）

```
✅ GitHub Actions: 直近5種success・期待4種cadence内
✅ Apify: 月換算 $8.35（窓 2026-06-15〜2026-06-21・上限 $12・on_target）
✅ Gemini: 2026-06-22 $0.097/日・直近4日平均で月換算 $6.1
✅ Buzz: 最終 2026-06-22（2h前）・status=pass・overlap=95.0%
✅ 収集品質: legal_rss=1/12（8%）・feeds 7/8健全・must_follow=6
✅ 分析構造: morning top=4/cat=4/action=4/fallback=0（item=12）
📋 未了TODO 4件（GH_PAT根本対応 / 法務Xクエリ追加 / IPWatchdog 504 / JURIST空）
```

### B. 確認したポイント

- **改善Bロールアウト成功（今日の重点）**: 6/21実装のfeed単位健全性計測が6/22(月)朝便で初反映＝`feeds 7/8健全`。8フィード中1つfailedは既知のIPWatchdog 504 or JURIST空（ops_backlogで管理・警告閾値「全フィード盲目」には未達＝合格）。「legal=0/1は新着なしであってフィード取得失敗ではない」を区別できる状態が本番稼働。
- **Buzz月曜便が正常稼働**: 6/22 02:10(月)のbuzz-collectが新コードでpass行を書き込み、最終行=6/22（2h前）・overlap=95%。週末ギャップ後の月曜便が設計どおり。
- **コスト横ばい合格**: Apify月$8.35（前日$9.17から低下方向・窓ずれ）、Gemini月$6.1。いずれもベースライン内。
- **改善A（must_follow急減検知）は沈黙が正常**: full便履歴は現在2件目（6/21・6/22）。判定に4件必要なため6/24(水)朝便以降に初有効化。6/22の沈黙は仕様どおり。
- **収集品質**: legal_rss=1/12（法務一色化なし）・must_follow=6（連続ゼロ回避）。分析構造もfallback0で正常。

### C. 追加対応＝JURIST死URL修正（改善Bが炙り出した実バグ）
日次チェック後の「他に気になる点」点検で、改善Bの `feeds 7/8健全` の非健全1枠＝JURISTの正体を特定・修正した。**改善B（feed単位の健全性計測）が無ければ feedparser は空フィードでも例外を投げず legal=0 と区別できないため、永久に気づけなかった盲点。**

| 項目 | 内容 |
|---|---|
| **原因** | `config.yaml` の JURIST 設定URL `https://www.jurist.org/feed/` は **HTTP 200を返すが空のスタブ(0件)**＝死んだ購読URL。これが ops_backlog「200だが恒常的に空」の正体。 |
| **検証** | 手動 `curl` で確認: 旧 `/feed/`＝200・size 1217・`<item>` **0件** / 新 `/news/feed/`＝200・size 101KB・`<item>` **15件**（最新 pubDate=6/21）。チャンネルtitle=「JURIST – News」で実ニュース確定。 |
| **修正** | `config.yaml:243` の url を `https://www.jurist.org/news/feed/` へ変更（コメントに経緯を併記）。YAML妥当性・feeds数=8 を確認済み。 |
| **確認待ち** | 法務RSSは毎朝便で走るので **6/23朝便で `legal_feeds_ok=7→8`（収集品質=feeds 8/8健全）** になれば反映成功＝ops_backlogのJURIST項目をクローズ。万一8にならなければUA/取得ロジック側を疑う。 |
| **副産物** | IPWatchdog 504 は **6/22 に自己回復**（朝便 `legal_feeds_failed=0`・手動curlも200で1件取得）＝一過性確定。再発なければ6/24頃クローズ予定（ops_backlog反映済み・監視のみ）。 |

> 📌 feed健全性の定義メモ（次担当者向け）: `legal_feeds_ok` は**entries>0のフィード数**、`legal_feeds_failed` は**HTTPエラー/例外のフィード数**。JURISTのような「200だが空」は **ok でも failed でもない**（=`7/8健全・failed=0` が両立する）。だから failed=0 を見て安心せず、`feeds N/8健全` の N を見ること。
...
- **未了の宿題**: `ops_backlog.yaml` を **4件→2件に削減**。①法務Xクエリ追加は**見送り削除**（次回アクション#5）②**GH_PAT根本対応は完全クローズ**（節F・デプロイ＆実テスト済み）。残る2件（IPWatchdog監視・JURIST反映待ち）はいずれも手当て済みで**6/23〜6/24に自動クローズ予定**＝新規の手作業なし。
- **GH_PAT運用の注意（次担当者）**: buzz「収集起動」ボタンは中継Worker `buzz-dispatch`（Cloudflare）経由。**PATはWorkerのsecretのみに存在**＝コード/公開ページには無い。ボタンが急に効かなくなったら、まずPATの有効期限切れ（fine-grained tokenの期限）を疑い、新PAT発行→Cloudflareダッシュボードでsecret `GH_PAT` 更新（手順 `workers/buzz-dispatch/README.md`）。Worker自体の再デプロイは不要。

### F. GH_PAT根本対応＝サーバー側中継化（実装→デプロイ→実起動テストまで完了・✅クローズ）
ops_backlogで唯一の「本物」だったGH_PAT問題（[[gh-pat-public-exposure]]）の**コード側を本日完了**した。

**問題（再掲）**: buzzの「収集起動」ボタンが、GitHub PATを**公開ページ docs/buzz.html に分割埋め込み**していた（誰でも復元可能＝第三者がワークフロー無断起動できる事故）。6/15にPATをrevoke＝止血済みだが、①埋め込む仕組みがコードに残存（新PATを入れたら再発）②ボタンは死んだまま、だった。

**実装（サーバー側中継方式）**: PATをブラウザに出さず、中継サーバー(Cloudflare Worker)の金庫(secret)に隠す。既存 `workers/post-generator`（Geminiキーを同方式で秘匿）と同じ流儀を踏襲。

| 変更 | 内容 |
|---|---|
| 新設 `workers/buzz-dispatch/index.js` | ハンドル名を受け取り `env.GH_PAT` で buzz-collect を dispatch する中継Worker。ハンドル検証(英数_・1〜15字)・days clamp・User-Agent付与済み。`node --check` 通過。 |
| 新設 `workers/buzz-dispatch/wrangler.toml`・`README.md` | デプロイ設定とユーザー向け手順書。 |
| `build_buzz.py` | PAT分割埋め込み(旧240-246行)を**撤去**し `const BUZZ_DISPATCH_URL` に置換。ブラウザの `addAccount()` を **api.github.com直叩き＋Authヘッダ → Worker POST(ハンドルのみ)** に変更。`build(gh_pat=)` 引数とenv読みも削除。 |
| `docs/buzz.html` | 再ビルドで**旧PAT `ghp_mLJz…` を公開ページから消去**（grep: ghp_=0・GH_PAT=0・api.github.com=0）。 |
| `.github/workflows/{buzz-collect,collect}.yml` | build_buzzが使わなくなった `env: GH_PAT` を削除。 |
...
### E. コミット台帳（2026-06-22・本セッション主要6件・全push済み／data自動commitは除く）
| # | hash | 種別 | 内容 |
|---|---|---|---|
| 1 | `d5aa5e4` | docs | 日次チェック実施＝全項目合格・改善Bロールアウト確認（feeds 7/8健全 初表示）。handoff冒頭＋本節A/B。 |
| 2 | `b3bc5ba` | fix(legal) | JURIST RSSを死URL `/feed/`→実フィード `/news/feed/` へ修正（本節C）。config.yaml・ops_backlog・handoff。 |
| 3 | `1118b2d` | docs | 引き継ぎを完成形に整備（冒頭整合・節順A〜E化・JURIST詳細表・feed健全性定義メモ・台帳）。 |
| 4 | `aecb65c` | ops | 法務X検索クエリ追加を見送り決定し ops_backlog から削除（4件→3件）。 |
| 5 | `e21c383` | feat(security) | GH_PAT根本対応の実装＝中継Worker `workers/buzz-dispatch/` 新設＋buzz.htmlからPAT撤去＋workflowのenv GH_PAT削除（本節F）。 |
| 6 | `0e1f1e3` | ops | GH_PAT完了クローズ＝Workerデプロイ＋secret格納＋実起動テスト確認、ops_backlogから削除（3→2件）。 |

※本コミット（台帳の最終確定）でhandoff全体の整合を仕上げ（冒頭サマリ・節タイトル・TL;DR・本節FをGH_PAT完了に更新）。working treeクリーン。

---

## 2026-06-21 日次チェック（全項目合格 / 変更1ロールアウト確認 ＋ 日次チェック改善5点A〜Eを実装＝daily_checkを6項目に拡張）

> このセッションでやったこと（2部構成・実施 6/21 04:20〜）:
> **第1部＝日次チェック**: ①`git pull --rebase`で6/21朝便分を取得 → ②`daily_check.py`で合否1画面確認（全✅） → ③6/21朝便ログに新フィールドが永続化され「収集品質」が実値表示になったことを確認（6/20実装=変更1のロールアウト成功）。
> **第2部＝改善A〜E**: 上のチェック表を見ながら「見落とし(=静かに壊れて気づけない穴)」を5つ潰した。A=must_follow急減検知 / B=法務RSSフィード健全性 / C=Actions鮮度(サイレント停止検知) / D=未了TODO可視化 / E=分析構造チェック。**daily_check.py は4項目→6項目＋TODOフッタに拡張。テスト 101→127件 全green。コミット13件（引き継ぎ整備・整合修正含む）・全push済み・working treeクリーン。**

> ### 🧭 次の担当者へ（まずこれだけ読めば動ける）
> - **現状**: 6/21は全項目合格。本セッションの改善5点は実装・push・テスト完了済み。**各改善の"本番データでの有効化"時期は異なる（重要）**:
>   - **C（Actions鮮度）= 本日すでに実データ確認済み**。今日の実走で fallback取得が Buzz Ranking を拾い `直近5種success・期待4種cadence内` を出した（=もう稼働中・待ち不要）。
>   - **E（分析構造）= 本日すでに稼働・合格**（6/21分析JSONを判定済み）。
>   - **B（法務feed健全性）= 6/22(月)朝便でロールアウト**（detail行に `feeds N/M健全` が初表示される）。
>   - **A（must_follow急減）= 当面は沈黙のまま**。判定には full便履歴が4件必要（`MUST_FOLLOW_DROP_MIN_PRIORS=3`＋当該便）だが現在full便は6/21の**1件のみ**。毎朝1件積み上がり、**4件目が入る6/24(水)朝便以降に初めて有効化**（6/22・6/23はまだ沈黙＝正常）。
>   - **D（未了TODO）= 本日すでに表示中**。
> - **明日(6/22)やること**: いつもどおり `python3 daily_check.py` を回し、**J節のチェック観点**で確認するだけ。注目は**Bの `feeds N/M健全` 初表示**。誤報リスクは確認済みで低い（A=履歴不足で沈黙・B=健全フィード6/8で警告閾値未満・C=月曜はbuzz稼働で新鮮）。普通に全項目合格になるはず。
> - **未了の宿題**: `ops_backlog.yaml`（改善Dで新設＝毎日チェック末尾に表示）に4件集約済み。**唯一の本物はGH_PAT根本対応**（セキュリティ債務・止血済み・別日に単独で）。他3点(IPWatchdog504/JURIST空/法務Xクエリ)は軽微・様子見。
> - **各改善の詳細・設計判断**: 下のE〜I節（A=E節, B=F節, C=G節, D=H節, E=I節）。コミット対応はK節。

### A. daily_check.py の結果（合否1画面・全✅）

**A-1. 初回スナップショット（改善前・04:20 JST・4項目）** — このセッション開始時に回した素の結果:
```
✅ GitHub Actions: 直近4種すべてsuccess        ← 改善C前は「窓に出た4種」だけ（Buzz Ranking不可視）
...
**A-2. 改善後の最終形（改善C/E/D反映後・05:00 JST・6項目＋TODOフッタ）** — このセッション終了時点で `python3 daily_check.py` が出す画面（=次担当が明日見るのと同じ形）:
```
✅ GitHub Actions: 直近5種success・期待4種cadence内    ← 改善C: Buzz Rankingを個別取得し明示確認
✅ Apify: 月換算 $9.17（窓 2026-06-14〜2026-06-20・上限 $12・on_target）
✅ Gemini: 2026-06-21 $0.127/日・直近4日平均で月換算 $6.7
✅ Buzz: 最終 2026-06-19（50h前）・status=pass・overlap=90.0%
✅ 収集品質: legal_rss=0/7（0%）・must_follow=5
✅ 分析構造: morning top=2/cat=2/action=4/fallback=0（item=7）  ← 改善E（新）
✅ 全項目合格
📋 未了TODO 4件（塩漬け防止・合否には影響しない）              ← 改善D（新フッタ）
  • [6d] GH_PAT のサーバー側中継 / [4d] 法務X検索クエリ / [0d] IPWatchdog504 / [0d] JURIST空
```
> 注: 6項目のうち**「収集品質」のfeed健全性(改善B)は6/22朝便からdetail行に出る**（6/21ログに新フィールドが無いため今は legal_rss/must_follow のみ表示）。「分析構造」は item=7 の低ニュース日だが構造的失敗なし＝正常（改善Eの設計どおり誤検知せず）。
...
### G. 改善Cを実装＝Actions鮮度チェック（スケジュールのサイレント停止を検知）
- **Cの正体**: `check_actions` は `gh run list` の窓（旧limit=15）に出た範囲の failure しか見ておらず、**ワークフローがサイレント停止（cron停止・60日無活動でActions無効化等）すると一覧から消えるだけで気づけない**。「直近N種success」は包括的に見えて実は**たまたま窓に入ったN種**だった。
- **実証**: 6/21(日)実測で `gh run list --limit 15` に出たのは pages/money/news/health の**4種だけ**。**月水金 cron の Buzz Ranking Collector（最終6/19金）が窓から押し出され不可視**（今日の連続pushでpagesビルドが窓を消費）。＝「直近4種success」にbuzz-collectが映っていなかった。
- **変更（daily_check.py）**: `EXPECTED_WORKFLOWS`（期待スケジュール4種＋最大許容age）を定義し、窓に出ない期待workflowは `--workflow` で個別取得（MWFも確実に拾う）。純関数 `evaluate_actions_freshness` で**不在（実行記録なし）/ stale（age超過）**を要確認に。failure は従来の横断スキャンが担当。出力を `直近N種success・期待4種cadence内` に変更。
- **許容age設計**: 毎日便（News/Money/Health）=26h（24h+2h余裕）、Buzz Ranking=84h（**月水金の週末ギャップ最大72h込み**＝Fri→Mon）。→ 週末は誤検知せず、84h超の本当の停止だけ捕まえる。
- **実走確認**: 修正後は `直近5種success・期待4種cadence内`＝**fallbackがBuzz Ranking Collectorを個別取得し、4種すべてcadence内と明示確認**（buzz-collect ~50h・週末ギャップ内）。従来の不可視が解消。
- **テスト**: 6ケース追加（全cadence内合格/不在検知/毎日便stale検知/週末ギャップ71hは誤検知せず/90hで停止検知/レジストリ網羅）。**全114件green**（108→114）。
- **副次の軽微ギャップ（未対応・許容）**: 当日04:5x発火のhealth-checkを04:20チェックでは見られない「タイミングずれ」は残るが、自己回復し翌日には判定される＝重要度低のため見送り（Cの本命はサイレント停止検知）。

### H. 改善Dを実装＝未了TODOを日次チェック末尾に表示（塩漬け防止）
- **Dの正体**: 未了の運用宿題（法務Xクエリ/GH_PAT根本対応/IPWatchdog504/JURIST空）が **handoffの散文の中に埋もれ忘れられやすい**（＝塩漬け）。アクティブなTODOだけ毎日視界に入れたい。
- **変更**: `ops_backlog.yaml` を新設（handoffは引き続き正本＝全経緯ログ、こちらは「いつかやる」アクティブ一覧）。`daily_check.py` に `load_backlog()` ＋ 純関数 `format_backlog(items, today)` を追加し、合否サマリの**下**に `📋 未了TODO N件` を**古い順（since経過日数つき）**で表示。
- **設計の要点**: **合否には影響しない**（exit code非変更＝TODOは「失敗」ではなく「催促」）。古いものが上に浮く＝放置が目立つ。`TODO_STALE_DAYS=30` 超で `⚠️` を付け軽く催促。読み込み失敗時は黙って非表示（チェック本体を壊さない）。
- **実走**: `📋 未了TODO 4件` 表示を確認（GH_PAT 6d / 法務Xクエリ 4d / IPWatchdog 0d / JURIST 0d）。完了したら `ops_backlog.yaml` の該当ブロックを削除する運用。
- **テスト**: 5ケース追加（空=非表示/古い順/経過日数と⚠️閾値/since欠落許容/note表示）。**全119件green**（114→119）。

### I. 改善Eを実装＝分析構造チェック（top/cat/action/fallback の自動判定）
- **Eの正体**: 分析JSONの構造完全性が手動確認だった（6/20 E節「分析構造指標の自動判定はまだ手動」）。`alerts.py` は**実行時にDiscord通知**で top_articles=0/fallback/save失敗を見るが、(1) **cat/action の空は未カバー**、(2) ローカルの `daily_check`（ファイル読み）からは再利用できない。
- **変更（daily_check.py）**: `load_latest_analysis()`（`data/analysis/` を日付→便順で最新選択）＋ 純関数 `evaluate_analysis_structure()` を追加し、6項目目として組込み。指標: top=`top_articles` / cat=`category_summaries` / action=`action_items` / fallback=`fallback_used_stages`。
- **設計のキモ＝ボリューム非依存**: 件数の多寡は**見ない**（低ニュース日は top/cat が少なくて正常）。実測でも 6/21(item=7)は top=2/cat=2/action=4 で正常。判定は**構造的失敗のみ**: ①fallback非空（429/5xxでモデル劣化）②top=0かつitem>0（掲載記事ゼロ＝図解も未生成）③top>0なのにcat=0（不整合）④item>0なのにaction=0（出力崩れ）。item=0（収集なし）の top=0 は当然合格。
- **実走**: `✅ 分析構造: morning top=2/cat=2/action=4/fallback=0（item=7）`＝低ニュース日を誤検知せず合格。
- **テスト**: 8ケース追加（健全/低ボリューム誤検知なし/fallback/top0+item>0/top0+item0許容/top>0でcat0/action0/記録なし）。**全127件green**（119→127）。daily_check は**6項目**に拡張（Actions鮮度・Apify・Gemini・Buzz・収集品質・分析構造）。

### J. 次回（6/22月）チェック観点
- **6/22(月)朝便の must_follow / total**（日曜要因か恒常縮小かの切り分け。非ゼロ・前日比で回復するか）。**改善Aの急減検知はまだ沈黙が正常**＝full便履歴を貯めている段階で、**有効化は4件目が入る6/24(水)朝便以降**（6/21=1件目→6/22=2→6/23=3→6/24=4）。それまでは「履歴不足で判定スキップ」が想定どおりの挙動。
- **6/22(月)朝便で「収集品質」detail行に `feeds N/M健全` が出るか**（改善Bのロールアウト確認＝今日の本命）。IPWatchdog/JURIST が継続failed/空なら feeds 6/8前後で安定するはず。
- **6/22(月)は buzz-collect稼働日**＝Actions鮮度（改善C・既に稼働中）で Buzz Ranking が ~数h に更新されるはず（平日挙動の確認）。
...
### K. このセッションのコミット（`feb7d2f^..HEAD` = 全13件・古い順）
| # | commit | 種別 | 内容 |
|---|--------|------|------|
| 1 | `feb7d2f` | docs | 2026-06-21 日次チェック（全項目合格・「収集品質」実値切替＝変更1ロールアウト確認） |
| 2 | `7fab3fa` | code | daily_check.py に must_follow急減検知を追加（full便のみ比較・履歴ゲート・テスト+4＝全105件green）＝**改善A** |
| 3 | `e6a610e` | code | 法務RSSのフィード健全性を計測＝legal=0の新着なし/取得失敗を分離（collector+main+daily_check・テスト+3＝全108件green）＝**改善B** |
| 4 | `617c683` | docs | 改善B を記録＝IPWatchdog504/JURIST空を検知 |
| 5 | `b11ee93` | code | Actions鮮度チェック追加＝期待スケジュールの不在/stale検知（daily_check・テスト+6＝全114件green）＝**改善C** |
| 6 | `026bc9b` | docs | 改善C を記録＝Buzz Ranking不可視を解消 |
| 7 | `5c8b6dc` | code | 未了TODOを daily_check 末尾に表示＝塩漬け防止（ops_backlog.yaml新設・テスト+5＝全119件green）＝**改善D** |
| 8 | `3f2105b` | docs | 改善D を記録＝ops_backlog.yaml運用開始 |
| 9 | `da0e190` | code | 分析構造チェックを daily_check に追加（top/cat/action/fallback・ボリューム非依存・テスト+8＝全127件green）＝**改善E** |
| 10 | `2f07d18` | docs | 改善E を記録（A〜E完了・daily_checkは6項目に） |
| 11 | `21f36e0` | docs | 引き継ぎを完成形に整備（次担当ガイド・daily_check最終形・変更ファイル一覧） |
| 12 | `ad440aa` | docs | 引き継ぎの整合修正（コミット台帳を全12件に補完=617c683/026bc9b追加・A/B/Cの有効化時期を正確化・初回/最終形の混在解消） |
| 13 | （本コミット） | docs | レビュー指摘の軽微表現を反映（冒頭サマリのActions表記を最終形=6項目に・row12を実hash ad440aaに確定） |

> 内訳: code 5件（A〜E本体）＋ docs 8件（日次記録＋各改善記録＋引き継ぎ整備2件）。**コード変更は全て daily_check.py 中心**（B のみ collector.py/main.py も触る）。

**daily_check.py の最終形（6項目＋フッタ）**＝次担当が `python3 daily_check.py` で見る画面:
1. GitHub Actions（横断failure＋**期待4種の不在/stale検知**=改善C）
2. Apify コスト（施策後フロア窓・月換算）
3. Gemini コスト（直近4日平均）
4. Buzz（メトリクス最終行の鮮度＋guardrail）
5. 収集品質（法務一色化・must_follow連続ゼロ・**must_follow急減**=改善A・**法務feed健全性**=改善B）
6. **分析構造（top/cat/action/fallback）=改善E**
+ **📋 未了TODO フッタ（`ops_backlog.yaml`・合否非影響）=改善D**

**新規ファイル**: `ops_backlog.yaml`（未了TODOの正本リスト・改善D）。**変更ファイル**: `daily_check.py`（A/B/C/D/E）, `collector.py`（B=feed健全性）, `main.py`（B=feed健全性の永続化）, `tests/test_daily_check.py`（+26ケース）。
