...
## 2026-06-22 日次チェック（全項目合格 / 改善Bロールアウト確認＝成功）
...
>
> **TL;DR（次担当者が最初に押さえる3点）**: (1) 6/22は**全項目合格**。(2) **JURIST死URLを修正済み**＝6/23朝便で `feeds 8/8健全` を確認したらクローズ。(3) 改善A（must_follow急減）は**6/24(水)で初有効化**、それまで沈黙が正常。
...
### F. GH_PAT根本対応＝サーバー側中継のコード実装（残りはデプロイのみ）
...
| F1 | `b3bc5ba` | fix(legal) | JURIST RSSを死URL `/feed/` から実フィード `/news/feed/` へ修正。config.yaml・ops_backlog.yaml・handoff(本節C)を更新。 |
| - | （台帳整備） | docs/ops | handoff構成整備（冒頭整合・節順化・台帳）＋法務Xクエリ見送り削除（4件→3件）。 |
| F2 | （本コミット） | feat(security) | GH_PAT根本対応＝中継Worker新設＋buzz.htmlからPAT撤去（本節F）。残りはデプロイのみ。 |
