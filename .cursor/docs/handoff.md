...
4. Gemini新機能バズ調査の追加期間・追加クエリ検討（専用収集は完成）
...
過去1年を6クエリ×25件で検索し、150件から17件を採用、既存分と合わせて
26件を累積ランキング化した。Apifyは`$0.0127`。厳格ルールと
Flash-Lite二次判定の両方を通過した投稿だけ採用する。英語投稿は日本語表示。
次回は26件を目視し、必要なら対象期間拡張または機能名別クエリを追加する。
