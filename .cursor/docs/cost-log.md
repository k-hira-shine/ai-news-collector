# Apify コスト比較ログ

## 2026-06-12（Buzz品質維持型削減テスト）

- 実装コミット: `47ceb10`
- 従来: 月・水・金すべて30日・100件。約`$0.1816/回`、月`$2.36`。
- テスト: 月・水は7日・50件、金曜は30日・100件で反応数と順位を再同期。
- バックテスト:
  - 取得件数 1,309件 → 410件（縮小日は68.7%減）
  - 新着保持率 99.0%
  - 30日URL保持率 99.0%
  - 上位20件一致率 100.0%
  - 欠落リスク 0件
- 月額見込み: `$1.28`、従来比約46%減。
- 2026-06-15、17の縮小実行と6月19日のフル再同期を比較する。
- 毎日JST 04:15の`Buzz Daily Health Check`で品質・鮮度を監視する。
- 詳細な合格条件と復旧手順は`.cursor/docs/handoff.md`先頭を参照。

## 2026-06-10（4施策の実装前基準）

### 実績
- 前請求サイクルの最終推定: **$47.91**
- 通常運用の比較期間: 2026-06-05〜2026-06-09
- 比較期間平均: **$0.7823/日**
- 月額換算: **$23.47**
- 内訳: News $4.19 / Money・SNS $13.87 / Buzz $5.42

### 採用する削減施策
1. SNS広域検索を3日に1回にし、検索クエリを整理する。
2. Money/SNSのアカウント取得を統合し、前回取得以降の差分収集にする。
3. Buzzを週3回にする。
4. News夕方便を検索と重要公式アカウント中心の軽量収集にする。

### 実装後の見込み
- 中心見込み: **$0.332/日、$9.96/月**
- 確認レンジ: **$8〜12/月**
- 基準比の見込み削減率: **57.6%**

見込みは施策間の重複を調整済み。Money/SNSの2施策は単純合算しない。
実装後は `data/cost_tracking.json` を各定期実行で更新し、完了済み7日間の
移動平均、月額換算、取得件数を確認する。詳細な前提は
`data/cost_reduction_plan.json` に記録する。

実装日当日の既存実行コストは比較対象から除外し、翌日の完了データから
削減後の移動平均を開始する。

## 2026-05-17（調整前）
| ワークフロー | コスト | 設定 |
|---|---|---|
| collect | $0.1254 | — |
| buzz | $0.2667 | max-items=200（デフォルト）、12アカウント |
| money | $0.7192 | max_items_per_query=100、24クエリ |
| **合計** | **$1.1113** | |

### 変更内容
- buzz: `--max-items 200 → 100`
- money: `max_items_per_query: 100 → 50`

## 2026-05-18（調整後）
| ワークフロー | コスト | 設定 |
|---|---|---|
| collect | $0.1291 | — |
| buzz | $0.1576 | max-items=100、12アカウント、1131件収集 |
| money | $0.6205 | max_items_per_query=50、24クエリ、683件収集 |
| **合計** | **$0.9072** | **▼$0.204（-18%）** |

## 2026-05-19（追加修正）

### 修正内容
- push競合対策: `buzz-collect.yml` / `collect.yml` の push リトライ時に `git stash` → `git pull --rebase` → `git stash pop` → 再commitする流れへ変更。
- cronコメント整理: GitHub Actions の cron コメントを JST 表記に統一。
- moneyコスト削減: `money_collection.search_interval_days: 2` を追加し、広域検索は2日に1回、追跡アカウントは毎日収集に変更。
- moneyクエリ削減: 広域検索クエリを17本 → 10本へ削減。

### 2026-05-20（push 競合の恒久対策）
- 3ワークフロー（collect / buzz / money）に `concurrency: repo-data-push` を設定し、main への書き込みを直列化。
- `.github/scripts/push_data.sh` を追加。rebase ではなく merge、ログ jsonl は行マージ、buzz.html は再ビルドで解決。

### 確認ポイント
- Buzz Ranking Collector が push 競合で失敗していないか。
- AI News Collector が push 競合で失敗していないか。
- money の `apify_cost_usd` が、広域検索ありの日で $0.25〜$0.35 前後に下がるか。
- 5/19 は広域検索スキップ日、5/20 は広域検索実行日の想定。
- money の Actor 起動数は、変更前 8回/日 → 変更後 平均3回/日が目安。
