...
"""
    ...
    prompt = f"""以下のXポスト一覧を分析し、「AIを使って稼いでいる・稼げた」という事例に関するポストを抽出してください。

## 抽出条件
以下のいずれかに該当するものを is_money_case: true とする：
- 本人またはAI活用した人が「収益を得た・稼いだ」という実績・報告
- 「こういうAIの使い方で稼げる」という具体的な手法紹介
- 「この人/このアカウントがAIで稼いでいる」という事例紹介・まとめ
- AIツールを使って副業・フリーランス・ビジネスをしているという事例

## 除外するもの（is_money_case: false）
- 単なるAIツールの紹介や感想（稼ぎに関係ない）
- 「AIで稼ごう」という呼びかけのみで実例なし
- リポスト（RTで始まるもの）
- 宣伝・広告のみ
...
- method: 稼ぎ方の手法（例：「Instagramアカウント運用」「AIコーディング代行」）
- tools: 使用AIツール（例：["Claude", "Gemini", "Kling"]）
